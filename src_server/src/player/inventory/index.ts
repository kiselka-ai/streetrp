...
class PlayerInventory {
	...
	constructor(storage: InventoryStorage) {
		...
		mp.events.subscribe({
			...
			'Inventory-SwapQuickSlots': this.swapQuickSlots.bind(this)
		});
	}
	...
	async addItem(
		...
		await this.storage.add(player, player.inventory, item, maxStack);
		await this.storage.updateInDb(player.dbId, player.inventory);
	}
	...
	private async useQuickItem(player: Player, slot: string) {
		...
		// Запоминаем быстрый слот, чтобы при снятии с рук (0) вернуть предмет обратно
		item.data = { ...item.data, quickSlotSource: slot };
	...
	private swapQuickSlots(player: Player, sourceSlot: string, targetSlot: string) {
		if (!equipment.isQuickSlot(sourceSlot) || !equipment.isQuickSlot(targetSlot)) {
			throw new SilentError('invalid quick slots');
		}
		if (sourceSlot === targetSlot) return;

		const itemA = equipment.getEquipment(player, sourceSlot);
		const itemB = equipment.getEquipment(player, targetSlot);

		equipment.setToSlot(player, sourceSlot, itemB ?? undefined);
		equipment.setToSlot(player, targetSlot, itemA ?? undefined);

		return { sourceSlot: itemB, targetSlot: itemA };
	}
	...
	private async unequipItem(player: Player, slot: string, cell?: number) {
		...
		// Снятие с рук по клавише 0: возвращаем в быстрый слот, если предмет был взят оттуда
		const quickSlotSource = item.data?.quickSlotSource;
		if (
			slot === 'hands' &&
			quickSlotSource &&
			equipment.isQuickSlot(quickSlotSource) &&
			!isNumber(cell)
		) {
			await equipment.unequip(player, item);
			equipment.setToSlot(player, quickSlotSource, item);
			delete item.data.quickSlotSource;
			return -1;
		}
