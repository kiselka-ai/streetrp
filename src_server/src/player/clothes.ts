...
class Clothes {
	...
	load(player: Player) {
		...
		Object.values(player.equipment).forEach((item) => {
			const itemData = inventoryHelper.getItemData(item.name);
			
			// Рюкзак обрабатывается отдельно через backpack.use(), не через clothes.set()
			if (itemData?.type === 'backpack') {
				// Рюкзак уже установлен функционально, нужно только визуально восстановить
				this.set(player, 'bag', { style: 31, color: 2 });
			} else if (item.data?.style >= 0 && itemData?.type === 'clothes') {
