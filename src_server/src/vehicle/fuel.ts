...
class VehicleFuel {
	...
	fillUp(vehicle: VehicleMp, amount: number) {
		...
		if (!fuel) {
			// Если топливо не установлено, инициализируем его
			const { tank } = vehicleCtrl.getTypeData(vehicle.name);
			vehicle.setVariable('fuel', { current: 0, max: tank });
			return this.fillUp(vehicle, amount); // Рекурсивно вызываем с инициализированным топливом
		}

		const total = Math.round((fuel.current || 0) + amount);
		const maxFuel = fuel.max || 100;

		vehicle.setVariable('fuel', {
			...fuel,
			current: total < maxFuel ? total : maxFuel
		});
	}
	...
	private decrease(vehicle: VehicleMp) {
		...
		if (fuel?.current > 0) {
			const reduced = fuel.current - 2; // Увеличен расход в 2 раза
