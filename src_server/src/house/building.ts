...
class HouseBuilding {
	...
	create(data: House) {
		...
		const entranceMarker = this.createPassage(data.position);
		this.createPassage(this.getExitPosition(data), dimension);

		if (coords.inventory) {
			this.createInventory(coords.inventory, dimension);
		}

		return {
			dimension,
			blip: !data.owner ? this.createBlip(data) : null,
			marker: entranceMarker // Сохраняем маркер входа для управления видимостью
		};
	}
	...
	enter(player: Player, house: House) {
		...
		const targetDimension = isEntrance ? house.dimension : 0;
	...
	private createInventory(position: PositionEx, dimension = 0) {
	...
	private createPassage(position: PositionEx, dimension = 0) {
		...
		const marker = mp.markers.new(2, new mp.Vector3(position.x, position.y, position.z), 1, {
			dimension,
			color: dimension ? [255, 255, 255, 120] : [255, 0, 156, 120],
			rotation: new mp.Vector3(180.0, 0.0, 0.0),
			visible: false // Изначально скрываем, показываем только для близких игроков
		});
