...
class HouseController {
	...
	private readonly MAX_DISTANCE = 50; // Максимальное расстояние видимости маркера в метрах

	constructor() {
		this.subscribeToEvents();
		this.startMarkerVisibilityCheck();
	}
	...
	private async buy(player: Player, entity: Entity) {
		...
		// Скрываем маркер для всех игроков после покупки
		if (entity.marker) {
			mp.players.forEach((playerMp) => {
				entity.marker.hideFor(playerMp);
			});
		}

		// Уведомляем клиент об обновлении домов
		player.callEvent('Player-HousesUpdated');
	}
	...
	private async sell(player: Player) {
		...
		// Показываем маркер для близких игроков после продажи
		if (house.marker) {
	...
	private async toggleLock(player: Player) {
		...
		const newLockedState = !house.locked;
		await entities.update(house, { locked: newLockedState });

		return newLockedState;
	}

	private startMarkerVisibilityCheck() {
		// Проверяем видимость маркеров каждые 2 секунды
	...
	private updateMarkersVisibility() {
		const availableHouses = entities.items.filter((house) => !house.owner && house.marker);

		mp.players.forEach((playerMp) => {
			const player = mp.players.get(playerMp);
			if (!player || !playerMp.position) return;

			availableHouses.forEach((house) => {
				if (!house.marker) return;

				const housePos = new mp.Vector3(house.position.x, house.position.y, house.position.z);
				const distance = playerMp.dist(housePos);

				const shouldBeVisible = distance <= this.MAX_DISTANCE;

				// Управляем видимостью маркера для конкретного игрока
				if (shouldBeVisible) {
					house.marker.showFor(playerMp);
				} else {
					house.marker.hideFor(playerMp);
				}
			});
		});
	}
