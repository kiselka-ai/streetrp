...
class Weapons {
	constructor() {
		mp.events.subscribe({
			...
			'Weapons-GetQuickSlotWeapon': this.getQuickSlotWeapon.bind(this),
			'Weapons-GetWeaponModel': this.getWeaponModel.bind(this)
		});
	}
	...
	getQuickSlotWeapon(player: Player, slot: string) {
		const item = equipment.getEquipment(player, slot);
		if (!item) return null;
		
		// Проверяем тип через inventoryHelper
		const itemData = inventory.getItemData(item.name);
		if (!itemData || itemData.type !== 'weapon') return null;
		
		return item.name;
	}

	getWeaponModel(player: Player, weaponName: string) {
		const weaponData = inventory.getItemData(weaponName);
		return weaponData?.model || null;
	}
}
