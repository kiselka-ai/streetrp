...
abstract class JobBranch {
	...
	protected setClothes(player: Player) {
		...
		// Сохраняем информацию о рюкзаке перед очисткой
		const backpack = equipment.getEquipment(player, 'backpack');
		...
		// Восстанавливаем рюкзак, если он был надет
		if (backpack) {
			playerClothes.set(player, 'bag', { style: 31, color: 2 });
		}
	}
