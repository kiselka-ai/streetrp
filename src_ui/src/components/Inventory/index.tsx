...
class Inventory extends Component<Props, State> {
	...
	async swapQuickSlots( sourceSlot: string, targetSlot: string ) {
		const { equipment } = this.props;

		try {
			const { sourceSlot: sourceItem, targetSlot: targetItem } = await rpc.callServer(
				'Inventory-SwapQuickSlots',
				[ sourceSlot, targetSlot ]
			);

			this.props.setEquipment( {
				...equipment,
				[ sourceSlot ]: sourceItem,
						...
						{storageState ? (
							...
							<>
								<Quick
									...
									swap={this.swapQuickSlots.bind( this )}
								/>
