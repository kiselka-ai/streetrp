import React, { Component } from 'react';
import rpc from 'utils/rpc';
import classNames from 'classnames';
import Controls from './controls';
import RPM from './rpm';
import Fuel from './fuel';
import Engine from './engine';

type Props = {
	binds: {
		[name: string]: string;
	};
};
type State = typeof initialState;

const initialState = {
	inVehicle: false,
	engine: {
		health: 100,
		active: false
	},
	velocity: 0,
	rpm: 0,
	gear: 0,
	fuel: {
		current: 0,
		max: 100
	},
	locked: false,
	seatbelt: false,
	cruise: false
};

export default class Speedometer extends Component<Props, State> {
	readonly state: State = initialState;

	componentDidMount() {
		rpc.register('Speedometer-UpdateState', (state: State) => this.setState(() => state));
	}

	componentWillUnmount() {
		rpc.unregister('Speedometer-UpdateState');
	}

	render() {
		const {
			inVehicle,
			engine,
			velocity,
			rpm,
			gear,
			fuel,
			cruise,
			locked,
			seatbelt
		} = this.state;

		return (
			inVehicle && (
				<div className={classNames('speedometer', { active: engine.active })}>
					<div className="speedometer_gear">
						{gear === 0 ? 'N' : gear}
					</div>
					<div className="speedometer_velocity">
						<span>{Math.round(velocity).toString().padStart(3, '0')}</span>

						<h5>КМ\Ч</h5>
					</div>

					<RPM amount={engine.active ? rpm : 0} />
					<Fuel amount={fuel.max > 0 ? (fuel.current * 100) / fuel.max : 0} />
					<Engine health={engine.health >= 0 && engine.health <= 100 ? engine.health : 0} />

					<Controls
						binds={this.props.binds}
						engine={engine.active}
						cruise={cruise}
						lock={locked}
						seatbelt={seatbelt}
					/>
				</div>
			)
		);
	}
}
