...
class HUD {
	...
	set visible(status: boolean) {
		...
		// Безопасный вызов - проверяем наличие браузера
		if (mp.browsers && mp.events && typeof mp.events.callBrowser === 'function') {
	...
	private toggle() {
		// Переключение HUD должно работать всегда, независимо от открытой страницы
		// Проверяем только наличие браузера через проверку возможности вызова события
		if (!mp.browsers) return;
