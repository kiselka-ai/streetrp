...
class VehicleSync {
	...
	syncTuning(vehicle: VehicleMp) {
		...
		// Сохраняем текущую радиостанцию перед применением тюнинга
		// Получаем текущую радиостанцию из игры, если игрок в транспортном средстве
		let savedRadioIndex: number | null = null;
		if (player.vehicle === vehicle && vehicle.getPedInSeat(-1) === player.handle) {
			const currentRadioIndex = mp.game.invoke('0xE8AF77C4C06ADC93');
			// Сохраняем только если радиостанция активна (не 255)
			if (currentRadioIndex !== 255) {
				savedRadioIndex = currentRadioIndex;
			}
		}

		// Применяем тюнинг
		vehicleTuning.set(vehicle, tuning);

		// Восстанавливаем радиостанцию после применения тюнинга с небольшой задержкой
		if (savedRadioIndex !== null) {
			setTimeout(() => {
				if (player.vehicle === vehicle && vehicle.getPedInSeat(-1) === player.handle) {
					mp.game.audio.setFrontendRadioActive(true);
					mp.game.audio.setRadioToStationIndex(savedRadioIndex);
				}
			}, 100);
		}
	}
